...
full_data.shape
full_data.info()
full_data['RainToday'] = full_data['RainToday'].map({'No': 0, 'Yes': 1})
full_data['RainTomorrow'] = full_data['RainTomorrow'].map({'No': 0, 'Yes': 1})
...
location = st.number_input("Location (encoded)", min_value=0, max_value=50, value=10)
...
wind_gust_dir = st.number_input("Wind Gust Dir (encoded)", min_value=0, max_value=15, value=3)
wind_gust_speed = st.number_input("Wind Gust Speed", value=40.0)
wind_dir_9am = st.number_input("Wind Dir 9am (encoded)", min_value=0, max_value=15, value=3)
wind_dir_3pm = st.number_input("Wind Dir 3pm (encoded)", min_value=0, max_value=15, value=3)
wind_speed_9am = st.number_input("Wind Speed 9am", value=15.0)
wind_speed_3pm = st.number_input("Wind Speed 3pm", value=20.0)
humidity_9am = st.number_input("Humidity 9am", value=70.0)
humidity_3pm = st.number_input("Humidity 3pm", value=60.0)
...
cloud_3pm = st.number_input("Cloud 3pm", value=4.0)
temp_9am = st.number_input("Temp 9am", value=22.0)
temp_3pm = st.number_input("Temp 3pm", value=28.0)
rain_today = st.selectbox("Rain Today?", ("No", "Yes"))
rain_today = 1 if rain_today == "Yes" else 0

# Collect inputs into same feature order
input_data = [[location, min_temp, max_temp, rainfall, evaporation, sunshine, wind_gust_dir,
...
# Use your best trained model for prediction (RandomForest for example)
if st.button("Predict"):
    prediction = model_rf.predict(input_data)[0]   # you can replace with model_cb / model_lgb / etc.
    if prediction == 1:
        st.success("☔ It will rain tomorrow!")
    else:
        st.info("🌞 No rain tomorrow.")
# import joblib

# # Save the trained model
# joblib.dump(model_rf, "rainfall_model.pkl")
